<?php
$conn = mysqli_connect("localhost", "root", "", "pwabaza2") or die('Greška pri spajanu sa bazom.');

if(isset($_POST["submit"])){
$username = trim($_POST["username"]);
$hashed = password_hash($_POST["password"], CRYPT_BLOWFISH);
$email = $_POST["email"];
...
if($exists != null){
    ...
    $sql = "INSERT INTO users (name, surname, email, username, pass, country) values (?,?,?,?,?,?)";
    if(mysqli_stmt_prepare($stmt, $sql)){
        mysqli_stmt_bind_param($stmt,'ssssss',$firstname,$lastname,$email,$username,$hashed,$country);
        mysqli_stmt_execute($stmt);
    }
...
<form action="" name="registration_form" method="POST">
		...
		<input type="text" id="lname" name="lastname" placeholder="Your last natme.." required=""><br>
