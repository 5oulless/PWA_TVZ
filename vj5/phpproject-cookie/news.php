<?php 
	...
	if (!isset($action)) {
		...
		if ($action == 1) {
			...
			$link = 'index.php?menu=' . $menu . '&action=1';
			setcookie("news_link_1", $link,time() + 86400 ,"/");
			...
			setcookie("news_title_2","Tech firm surge pushes US markets higher",time() + 86400 ,"/");
			$link2 = 'index.php?menu=' . $menu . '&action=2';
			setcookie("news_link_2", $link2,time() + 86400 ,"/");
			# inicijalizacija kolačića
			print '
			...
			$link3 = 'index.php?menu=' . $menu . '&action=3';
			setcookie("news_link_3", $link3,time() + 86400 ,"/");
